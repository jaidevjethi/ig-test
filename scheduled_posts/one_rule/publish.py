...
def get_asset_url(filename):
    return f"https://raw.githubusercontent.com/{REPO_NAME}/{BRANCH}/scheduled_posts/one_rule/slides/{filename}"

def publish():
    print("🚀 Initializing Cloud Publication for Carousel 6...")
    
    # 1. Load Caption & Comment
    with open("scheduled_posts/one_rule/caption.txt", "r", encoding="utf-8") as f:
        caption = f.read().strip()
    
    # 2. CREATE IMAGE CONTAINERS
    container_ids = []
    slides = [f"slide_{i:02d}.png" for i in range(1, 9)]
    ...
    print("  [CAROUSEL] Assembling 8-slide carousel...")
    res = requests.post(f"{BASE_URL}/media", data={
        "caption": caption,
        "media_type": "CAROUSEL",
    ...
    if post_id:
        ...
        with open("scheduled_posts/one_rule/comment.txt", "r", encoding="utf-8") as f:
